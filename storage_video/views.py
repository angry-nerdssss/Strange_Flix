...
def video(request,id):
    ...
    if(current_user.is_superuser):
        ...
        if Time.objects.filter(username=current_user.username).filter(video_title=video.title).exists() :
            obj=Time.objects.filter(username=current_user.username).filter(video_title=video.title)[:1].get()
...
def svideo_like(request):
    ...
    ctx = {'likes_count': video.total_likes, 'message': message,'dislikes_count': video.total_dislikes,'liked':liked,'disliked':disliked}
...
def svideo_dislike(request):
    ...
    ctx = {'dislikes_count': video.total_dislikes, 'message': message,'likes_count': video.total_likes,'liked':liked,'disliked':disliked}
    # use mimetype instead of content_type if django < 5
    return HttpResponse(json.dumps(ctx), content_type='application/json')
    return HttpResponseRedirect(reverse('play_svideo', args=[str(video.id)]))
...
def getCurrentTime(request):
    ...
    if Time.objects.filter(username=current_user.username).filter(video_title=title).exists() :
        time=Time.objects.filter(username=current_user.username).filter(video_title=title)[:1].get()
        time.currentTime=current_time
        time.save()
    else :
        time=Time.objects.create(username=current_user.username,video_title=title,currentTime=current_time)
    ctx={}
    return HttpResponse(json.dumps(ctx), content_type='application/json')
